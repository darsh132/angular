...
export class BoardComponent implements OnInit {
  ...
  private persistMove(issue: Issue, status: IssueStatus): void { const previous = this.issues(); this.issues.set(previous.map(x => x.id === issue.id ? { ...x, status } : x)); this.moving.set(issue.id); this.api.move(issue.id, status).subscribe({ next: () => this.moving.set(null), error: e => { console.error(e); this.issues.set(previous); this.moving.set(null); } }); }
  ...
  createIssue(): void { if (!this.newIssue.title.trim()) return; this.saving.set(true); this.api.create({ ...this.newIssue, title: this.newIssue.title.trim() }).subscribe({ next: () => { this.createOpen.set(false); this.load(); }, error: e => console.error(e), complete: () => this.saving.set(false) }); }
