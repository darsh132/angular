...
export interface BurndownPoint { date:string; remainingPoints:number; }
export interface SprintAnalytics { sprintId:number; name:string; status:string; startDate:string; endDate:string; committedPoints:number; completedPoints:number; remainingPoints:number; issueCount:number; completedIssueCount:number; ideal:BurndownPoint[]; actual:BurndownPoint[]; }
...
export class JiraApiService {
  ...
  sprintAnalytics(projectId:number,sprintId:number):Observable<SprintAnalytics>{return this.http.get<SprintAnalytics>(`${this.baseUrl}/projects/${projectId}/sprints/${sprintId}/analytics`);}
