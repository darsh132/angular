...
export interface UserOption { id:number; name:string; avatar:string; }
...
export interface UpdateIssueRequest { title:string; description:string; priority:IssuePriority; type:IssueType; storyPoints:number; assigneeId?:number; sprintId?:number; }
...
export class JiraApiService {
  ...
  users(): Observable<UserOption[]> { return this.http.get<UserOption[]>(`${this.baseUrl}/users`); }
  ...
  update(id:number, request:UpdateIssueRequest): Observable<void> { return this.http.put<void>(`${this.baseUrl}/issues/${id}`, request); }
