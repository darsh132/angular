...
builder.Services.AddHttpContextAccessor(); builder.Services.AddScoped<IssueApplicationService>(); builder.Services.AddScoped<IssueMetadataApplicationService>(); builder.Services.AddScoped<SprintApplicationService>(); builder.Services.AddScoped<SprintAnalyticsService>(); builder.Services.AddScoped<ProjectDashboardService>(); builder.Services.AddScoped<AuthService>(); builder.Services.AddScoped<ProjectAuthorizationService>();
...
builder.Services.AddCors(o => o.AddPolicy("frontend", p => p.WithOrigins("http://localhost:4200", "https://localhost:4200").AllowCredentials().AllowAnyHeader().AllowAnyMethod())); builder.Services.AddProblemDetails(); builder.Services.AddControllers(); builder.Services.AddEndpointsApiExplorer(); builder.Services.AddSwaggerGen();
